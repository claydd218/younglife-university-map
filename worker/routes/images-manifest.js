...
// in images/, in one request. bigtime/reports/reports.js and
// bigtime/reports2/reports2.js both need this to resolve each staff
// member's photo: staff photos aren't stored with a known filename the way
// a ministry's own `photos` column is (see lib/ministries.js), only a
// slugified name, so the actual extension (png/jpg/jpeg/webp — CONFIG.
// IMAGE_EXTENSIONS) has to be guessed. Both report pages used to guess by
// firing a HEAD request per candidate extension per staff member — with
